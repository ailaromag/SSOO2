...
/**
 * Programa principal que crea un nuevo archivo en el sistema de archivos.
 * Similar al comando touch de Unix, crea ficheros con permisos específicos.
 * 
 * Parámetros de entrada:
 ...
 *         y argv[3] la ruta del fichero a crear (no debe terminar en '/')
 * 
 * Return:
 * - EXITO (0) si crea el fichero correctamente
 * - FALLO (-1) si error en argumentos, permisos inválidos, ruta termina en '/',
 *              montaje, creación del fichero o desmontaje
 */
int main(int argc, char **argv) {
    ...
    // Extraer la ruta del fichero a crear
    const char *camino = argv[3];
    // Verificar que la ruta no termina en '/' (requisito para ficheros)
    ...
    // Intentar crear el fichero con los permisos especificados
